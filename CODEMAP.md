...
- `src/classifieds_hub/collectors/tg_chat.py`
  - сбор сообщений из публичных Telegram чатов через Client API (Telethon)
  - щадящий backfill/инкремент + классификация сообщений в объявления
  - для медиа сохраняет `tgmsg://chat_ref/message_id` (без локального скачивания)
...
- `listing_media`
  ...
  - для Telegram источника хранит `tgmsg://...` ссылки

- `tg_raw_messages`
  - сырой слой сообщений из Telegram чатов (для анализа/фильтрации)

- `source_cursors`
  - курсоры источников (например последний `message_id` в чате)
...
- 2026-02-21:
  ...
  - Добавлен Telegram Client API коллектор для публичных чатов (`tg_chat`) с raw-слоем и курсорами.
